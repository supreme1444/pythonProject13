...
from app.app_services.user_services import services_user_check
...
async def create_referral_code(expiration_days: int, db: AsyncSession = Depends(get_db), user: models.User = Depends(auth.get_current_user)):
